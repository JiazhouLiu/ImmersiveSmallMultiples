...
public class PupilManager : MonoBehaviour 
{
	...
	{	
        ...
        if (GameObject.Find("[CameraRig]") != null)
        {
            ...
            Debug.Log("cannot find steamvr camera!");
            PupilSettings.Instance.currentCamera = GetComponentInChildren<Camera>();
        }
	...
	void ShowCalibrate()
	{
		calibrationText.text = "Now, we will calibrate your eyetracking data.\n\nPlease keep your headset stable.\nLater, you will see a dot in the screen.\nPlease follow that dot until finish.\n\nPress <color=red>trigger</color> button to start calibration.";
	}
	...
	{
		calibrationText.text = "Now, we will calibrate your eyetracking data.\n\nPlease keep you headset stable.\n\nPress <color=red>trigger</color> button to start calibration.";
	...
	{
		calibrationText.text = "Calibration failed\nPress <color=red>trigger</color> button to start it again.";
	...
	void StartTask()
	{
        ...
        {
            Debug.LogError("cannot find small multiples manager");
        }
    }
