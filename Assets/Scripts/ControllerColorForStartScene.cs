...
public class ControllerColorForStartScene : MonoBehaviour {
    ...
    void Update () {
        ...
        if (!touchBarFlag) {
            ...
            if (touchbar != null)
            {
                ...
                touchbar.transform.GetChild(0).GetComponent<CollisionDetection>().enabled = false;
                touchBarFlag = true;
            }
        }
        ...
        if (this.transform.childCount > 0 && !triggerColorFlag)
        {
            ...
            GameObject lrLabel = (GameObject)Instantiate(LRLabelPrefab, new Vector3(0, 0, 0), Quaternion.identity);
