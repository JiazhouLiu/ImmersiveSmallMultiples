	...
	void Update ()
	{
        ...
        if (GameObject.Find("SmallMultiplesManager") != null) {
            ...
            if (!smms.afterCalibration) {
                ...
                // customise keyboard C
                //if (GameObject.Find("[VRTK_SDKManager]") != null)
                //{
                if (PupilTools.IsConnected && ltc != null && rtc != null && (ltc.triggerPressed || rtc.triggerPressed))
                {
                    if (PupilTools.IsCalibrating)
                    {
