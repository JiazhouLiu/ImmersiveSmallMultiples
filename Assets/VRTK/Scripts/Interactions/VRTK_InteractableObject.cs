    ...
    public class VRTK_InteractableObject : MonoBehaviour
    {
        ...
        public virtual void ToggleHighlight(bool toggle)
        {
            ...
            if (touchHighlightColor != Color.clear && objectHighlighter)
            {
                ...
                {
                    objectHighlighter.Highlight(grabColor);
                }
                else
                {
        ...
        {
            if (GetTouchingObjects().Count > 0)
            {
                if ((GetTouchingObjects().Count == 1))
                {
                    if (GetTouchingObjects()[0].name.Equals("LeftController"))
                    {
                        Color color = Color.blue;
                        grabColor = color;
                        color.a = 0.5f;
                        touchHighlightColor = color;
                        ToggleHighlight(true);
                    }
                    if (GetTouchingObjects()[0].name.Equals("RightController"))
                    {
